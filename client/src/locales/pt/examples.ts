...
const factorialCode = `int fatorial(int n) {'{'}
  if (n <= 1) {'{'}
    return 1;
  {'}'} else {'{'}
...
const tictactoeCode = `void iniciarTabuleiro(int[3][3] tabuleiro) {'{'}
  int i, j;
...
const hangmanCode = `void iniciarPalavraSecreta(char[10] palavraSecreta) {'{'}
